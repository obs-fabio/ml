	...
	for class_id in ['A', 'B', 'C', 'D']:
		...
		for i_fold, (train_dataset, val_dataset, test_dataset) in enumerate(config.get_dataset_loro()):
			...
			for training_dict in training_dicts:
				...
				bin_not_selecteds = ~np.isin(np.arange(len(kl_gan)), bin_selections)

				kl_gan_sum = np.median(kl_gan[bin_not_selecteds])
				kl_ganspe_sum = np.median(kl_ganspe[bin_not_selecteds])
		...
		if not one_fold_only:
			...
			fold_kl_divergence = np.median(fold_kl_divergence, axis=0)
			fold_kl_gan = np.median(fold_kl_gan, axis=0)
			fold_kl_ganspe = np.median(fold_kl_ganspe, axis=0)
			fold_data_real = np.median(fold_data_real, axis=0)
			fold_data_gan = np.median(fold_data_gan, axis=0)
			fold_data_ganspe = np.median(fold_data_ganspe, axis=0)
			...
			kl_gan_sum_spe = np.median(fold_kl_gan[bin_selections[class_id]])
			kl_ganspe_sum_spe = np.median(fold_kl_ganspe[bin_selections[class_id]])
			kl_factor_spe = kl_ganspe_sum_spe/kl_gan_sum_spe
			
			kl_gan_sum = np.median(fold_kl_gan)
			kl_ganspe_sum = np.median(fold_kl_ganspe)
			kl_factor = kl_ganspe_sum/kl_gan_sum
			...
			ax3.plot(kl_x, fold_kl_divergence, label = "norm")
			ax3.axhline(1, color='red', linestyle='--')
			ax3.set_ylim([0,2])
			ax3.legend()
			...
			ax1.set_title(f"{kl_factor} -> {kl_factor_spe}")
			fig.tight_layout()
			plt.savefig(os.path.join(training_plot_dir, f'{class_id}_kl.png'))
			plt.close()
