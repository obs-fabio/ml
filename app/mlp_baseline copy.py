...
for i_fold, (train_dataset, val_dataset, test_dataset) in tqdm.tqdm(enumerate(config.get_dataset_loro()), desc="Folds", leave=False):
    ...
    if train:
        ...
        for id, class_id in enumerate(class_list):
            ...
            if os.path.exists(model_file):
                ...
                trainings = {
                    str(config.Training.BASELINE): {
                        'dataset': train_dataset,
                        'file': model_file,
                        'loss_file': model_loss_file
                    }
                }

            for source in source_synthetics:

                model_file = os.path.join(baseline_model_dir, f'{class_id}_{str(source)}_model.pkl')
                model_loss_file = os.path.join(baseline_model_dir, f'{class_id}_{str(source)}_model_loss.png')

                if os.path.exists(model_file):
                    continue

                syn_train_dataset, syn_val_dataset, syn_test_dataset = ml_data.load_synthetic_dataset(
                        config.get_result_dir(i_fold, source, config.Artifacts.OUTPUT),
                        train_dataset.transform)

                trainings[str(source)] = {
                        'dataset': syn_train_dataset,
                        'file': model_file,
                        'loss_file': model_loss_file
                    }

            for train_id, dict in trainings.items():

                image_shape = list((dict['dataset'].__getitem__(0)[0]).shape)
    ...
    if evaluate:
        ...
        for id, class_id in enumerate(class_list):
            ...
            train_file = os.path.join(baseline_output_dir, f'model({str(config.Training.BASELINE)})_eval({str(config.Training.BASELINE)})_train.csv')
            val_file = os.path.join(baseline_output_dir, f'model({str(config.Training.BASELINE)})_eval({str(config.Training.BASELINE)})_val.csv')
            test_file = os.path.join(baseline_output_dir, f'model({str(config.Training.BASELINE)})_eval({str(config.Training.BASELINE)})_test.csv')
            ...
            if os.path.exists(train_file) and \
                ...
                for class_id in class_list:
                    classifier_file = os.path.join(baseline_model_dir, f'{class_id}_{str(config.Training.BASELINE)}_model.pkl')
                    evals['classifiers'].append(ml_model.Serializable.load(classifier_file))
